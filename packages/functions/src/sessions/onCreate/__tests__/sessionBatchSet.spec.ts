...
import sessionBatchSet from "../sessionBatchSet";
...
test("Invoke a request to batch set the session document", async done => {
  ...
  const wrapped = testEnv.wrap(sessionBatchSet);
  ...
  expect(db.batch().set).toHaveBeenCalledWith("roomRef", memberSessionResponse);
