...
import InviteFormSignupStyles from "./InviteFormSignupStyles";
...
export default function InviteFormSignup({
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  inviteId,
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  spaceId,
}: Props): JSX.Element {
  const classes = InviteFormSignupStyles();
  const {backdrop} = useBackdrop();
  const {snackbar} = useSnackbar();
  const {t, lang} = useTranslation();
  const {query} = useRouter();

  const InviteFormSignupSchema = Yup.object().shape({
    email: Yup.string()
  ...
  const {control, register, errors, handleSubmit} = useForm({
    ...
    resolver: yupResolver(InviteFormSignupSchema),
  });
